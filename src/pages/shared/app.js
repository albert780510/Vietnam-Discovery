...
async function main(locale){
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // NEW: confirmation step (避免填錯就直接進付款)
    const confirmTitle = isEn ? 'Confirm your info' : (isZhCn ? '确认资料' : '確認資料');
    const confirmBtn = isEn ? 'Confirm & continue' : (isZhCn ? '确认后继续' : '確認後繼續');
    const backBtn = isEn ? 'Back to edit' : (isZhCn ? '返回修改' : '返回修改');

    function esc(s){
      return String(s||'').replace(/[&<>"']/g,(c)=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[c]));
    }

    // Build a small review modal.
    const overlayId = 'vd-confirm-overlay';
    ...
    if (!overlay) {
      ...
      overlay.style.display = 'none';
      overlay.innerHTML = `
        <div data-vd-confirm-backdrop style="position:absolute;inset:0;background:rgba(0,0,0,.45);backdrop-filter:blur(6px)"></div>
        <div style="position:relative;margin:0 auto;max-width:560px;padding:16px;top:8vh">
          <div style="border-radius:24px;border:1px solid rgba(0,0,0,.10);background:rgba(255,255,255,.92);box-shadow:0 18px 60px rgba(0,0,0,.22);padding:16px">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px">
              <div>
                <div style="font-size:16px;font-weight:900;color:#18181b" id="vd-confirm-title"></div>
                <div style="margin-top:6px;font-size:13px;color:#52525b">${isEn?'Please double-check before we create the order.':(isZhCn?'创建订单前请再次确认。':'建立訂單前請再次確認。')}</div>
              </div>
              <button type="button" id="vd-confirm-close" class="vdBtnAlt">${backBtn}</button>
            </div>
            <div id="vd-confirm-body" style="margin-top:12px;font-size:14px;color:#3f3f46;line-height:1.6"></div>
            <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:14px">
              <button type="button" id="vd-confirm-back" class="vdBtnAlt">${backBtn}</button>
              <button type="button" id="vd-confirm-go" class="btn">${confirmBtn}</button>
            </div>
          </div>
        </div>
      `;
      ...
      overlay.addEventListener('click', (ev)=>{
        if (ev.target && ev.target.getAttribute && ev.target.getAttribute('data-vd-confirm-backdrop') !== null) {
          overlay.style.display = 'none';
        }
      });
      overlay.querySelector('#vd-confirm-close')?.addEventListener('click', ()=>{ overlay.style.display='none'; });
      overlay.querySelector('#vd-confirm-back')?.addEventListener('click', ()=>{ overlay.style.display='none'; });
    }

    const titleEl = overlay.querySelector('#vd-confirm-title');
    const bodyEl = overlay.querySelector('#vd-confirm-body');
    if (titleEl) titleEl.textContent = confirmTitle;
    if (bodyEl) {
      bodyEl.innerHTML = `
        <div style="display:grid;gap:6px">
          <div><b>${isEn?'Email':(isZhCn?'邮箱':'Email')}</b>：${esc(payload.email)}</div>
          <div><b>${isEn?'Phone':(isZhCn?'电话':'電話')}</b>：${esc(payload.phone)}</div>
          <div><b>${isEn?'Arrival date':(isZhCn?'入境日期':'入境日期')}</b>：${esc(payload.arrival)}</div>
          <div><b>${isEn?'Nationality':(isZhCn?'国籍':'國籍')}</b>：${esc(payload.nationality)}</div>
          <div><b>${isEn?'Entry gate':(isZhCn?'入境口岸':'入境口岸')}</b>：${esc(payload.entryGate)}</div>
          <div><b>${isEn?'Address (English)':(isZhCn?'英文地址':'英文地址')}</b>：${esc(payload.address)}</div>
        </div>
      `;
    }

    // Wire confirm action
    const goBtn = overlay.querySelector('#vd-confirm-go');
    const proceed = async () => {
      overlay.style.display = 'none';
      ...
      try {
          ...
          dlg.innerHTML = `
            <form method="dialog" class="vd-modal__card">
              ...
              <div class="vd-modal__body" style="white-space:normal">
                <div class="sub" style="margin-bottom:10px">${isEn ? 'Pick one method to continue.' : (isZhCn ? '请选择一种付款方式继续。' : '請選擇一種付款方式繼續。')}</div>
                <div class="vdPayGrid">
                  <button type="button" class="vdPayBtn" data-method="TWD"><span>TWD 转账</span><span class="sub">${fmtMoney(tTwd.total,'TWD',locale)}</span></button>
                  <button type="button" class="vdPayBtn" data-method="CNY"><span>CNY 支付宝/微信</span><span class="sub">${fmtMoney(tCny.total,'CNY',locale)}</span></button>
                  <button type="button" class="vdPayBtn" data-method="USDT"><span>USDT (BEP20)</span><span class="sub">${fmtMoney(tUsdt.total,'USDT',locale)}</span></button>
                </div>
              </div>
              <div class="vd-modal__actions">
                <button class="vdBtnAlt" value="cancel">${isEn ? 'Cancel' : (isZhCn ? '取消' : '取消')}</button>
              </div>
            </form>
          `;
          document.body.appendChild(dlg);
        ...
        if (!picked || picked === 'cancel') {
          // Re-enable submit so they can try again.
          if (submitBtn) submitBtn.disabled = false;
          return;
        }

        // Reveal payment method details
        if (payMethod) payMethod.value = picked;
        payMethod?.dispatchEvent(new Event('change', { bubbles: true }));

        // Scroll to payment section
        (paymentBox || proofBox)?.scrollIntoView?.({ behavior: 'smooth', block: 'start' });

      } catch (err) {
        if (submitBtn) submitBtn.disabled = false;
        await setStatus('danger', isEn
          ? 'Failed to create order. Please try again or contact us.'
          : (isZhCn ? '建立订单失败，请重试或联系。' : '建立訂單失敗，請重試或聯繫。')
        );
      }
    };
